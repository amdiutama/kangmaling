...
def toketz():
	os.system('clear')
	print (' %s*%s tools ini menggunakan login toket facebook.\n %s*%s apakah kamu sudah tau cara mendapatkan toket facebook?\n %s*%s ketik %sopen%s untuk mendapatkan toket facebook.'%(O,N,O,N,O,N,H,N))
	toket = raw_input('\n %s[%s?%s] toket :%s '%(N,M,N,H))
	if toket in ('open', 'Open', 'OPEN'):
		...
		logo()
	try:
		otw = requests.get('https://graph.facebook.com/me?access_toket=%s'%(__takondy__))
