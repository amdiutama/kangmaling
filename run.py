...
### SETTING USER AGENT
def ua():
	print('\n\t[ Useragent Setting ]\n\n# Useragent Saat Ini:',open('ua','rex').read(),'\n')
	...
	if ask =='':
		...
		try:
			...
			raw_input('Enter For Back To Menu')
			time.sleep(2)
			menu()
			exit()
	else:
...
def tokenz():
	...
	try:
		...
		logo()
		print("\n "+N+"["+K+"*"+N+"] How To Get Token : https://youtu.be/RIpCHs7E4qs")
		token = raw_input(" "+N+"["+M+"*"+N+"] Your Token : "+H+"")
		try:
			...
			avsid.write(token)
			avsid.close()
			print(" "+N+"["+H+"*"+N+"] Login Successfully")
			bot_komen()
		except KeyError:
