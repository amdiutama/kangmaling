...
def anjinganjinganjingerrorerrorerroranjinganjinganjingerrorerrorerror():
        ask = raw_input('\n [*] menu : ')
        if ask == '':
           print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu tolol!'%(N,M,N,M,ask,N);time.sleep(1);os.system('clear');menu()
        elif ask =='1':
                __ahya_ganteng()
        elif ask =='2':
                _dds()
            ...
            print '\n %s[%s×%s] menu [%s%s%s] tidak ada mungkin salah ketik'%(N,M,N,M,ask,N);time.sleep(1);os.system('clear');menu()
def logo():
	try:
		token = open('login.txt', 'r').read()
	except (KeyError, IOError):
...
def __ahya_ganteng():
	...
	r = requests.get('https://graph.facebook.com/%s/friends?limit=%s&access_token=%s'%(token)
	z = json.loads(r.text)
	for i in z['data']:
		uid = i['id']
		na = i['name']
