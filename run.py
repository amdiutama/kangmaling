...
def public():
	...
	for i in z["data"]:
		...
		name = i['name']
		id.append(uid+'<=>'+name)
	print(" \033[0;97m[\033[0;93m*\033[0;97m] total target  : "+str(len(id)))
...
def manual():
	...
	def main(user):
		global loop, token
		sys.stdout.write('\n\r \033[0;97m[\033[0;93m*\033[0;97m] cracking %s/%s ok-:%s - cp-:%s ' % (loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		try:os.mkdir("results")
		except OSError:pass
		uid,name=user.split("<=>")
