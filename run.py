...
def public():
	...
	ask = raw_input("\n \033[0;97m[\033[0;93m?\033[0;97m] Do you want to use manual password\x1b[1;91m?] Y/t : ")
	if ask == "Y" or ask == "y":
		manual()
	...
	def main(user):
		...
		pwx = []
		sys.stdout.write(
		      '\r \x1b[0;97m[\x1b[0;96m%s\x1b[0;97m] Cracking %s/%s OK-:\033[0;92m%s] - CP-:\033[0;92m%s] ' % (datetime.now().strftime('%H:%M:%S'), rgb,loop, len(id), len(ok), len(cp))
		); sys.stdout.flush()
		uid,name=user.split("<=>")
...
### SETTING USER AGENT
def settingua():
	ask = raw_input("\n \033[0;97m[\033[0;93m?\033[0;97m] Do you want to change the User Agent? [Y/t] : ") 
	if ask =="":
		menu()
	elif ask == "y" or ask == "Y":
		try:
